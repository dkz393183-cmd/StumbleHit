...
function updateUserIcon(userData) {
    ...
    userIcons.forEach(userIcon => {
        if (userData.photoURL) {
            userIcon.innerHTML = `<img src="${userData.photoURL}" alt="User" style="width: 55px; height: 55px; border-radius: 50%; border: 2px solid #000; cursor: pointer;">`;
        } else {
            userIcon.innerHTML = `<div style="width: 55px; height: 55px; border-radius: 50%; background: linear-gradient(135deg, #667eea, #764ba2); display: flex; align-items: center; justify-content: center; border: 2px solid #000; color: white; font-size: 24px; cursor: pointer;">${userData.displayName.charAt(0)}</div>`;
        }
